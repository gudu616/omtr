# Corpus-verified memorization tracks output sharpness — a preregistered crossed control then finds a dose–response of similar strength on never-memorized predictable text, without resolving whether the effect is memorization-specific — and threshold-based "convergence depth" turns out to be a threshold artifact not separable from it: a pilot and its self-tests in two open-corpus model families (0.4–3B)

*Draft v0.7 = draft v0.6 (published as v2 of the concept DOI 10.5281/zenodo.22039215) plus the preregistered L0P crossed control that v0.6 listed as its one open identification gap. The preregistration (`docs/PREREG_L0P.md`, public commit `459710c`) was posted before any L0P measurement existed; the analysis code was written and frozen, and its interpretive rules pre-committed in the process log, before anyone saw a result. The v0.6 text is preserved below unrewritten, except for one bracketed v0.7 note in "What stands" item 1. 2026-08-21.*

*Correction (2026-08-21): the model-external anchor that this document's headline
sections name "corpus duplication counts" is, operationally, a median probe-window
frequency — in practice dominated by the passage's most-repeated short phrases —
not a count of passage copies (in Pile, the median complete-copy count of the
17 gate-eligible passages is 18, below the gate's own threshold of 20). The
methods section describes the window mechanism correctly; the headline name
overstates it. Full statement, per-corpus numbers, and reproduction script:
`CORRECTION_20260821_ANCHOR.md`. No registered verdict changes.*

## The v0.7 preregistered crossed control: no registered verdict arrived — read this first

v0.6's surviving claim was a dose–response within corpus-verified memorized text: teacher-forced gold log-probability against the final-layer entropy of the model's free continuation, rho −0.70 to −0.82 in all five runs. v0.6 also named the gap in that claim: its control was matched on construction, not on predictability (gold-range overlap only 13–27%), so the data could not say whether the dose–response is **memorization-specific** or simply **generic predictability** — any sufficiently predictable text producing the same sharpness. The L0P battery was built to cross that gap: 18 authored English paragraphs whose referring nouns are invented lexemes carried by an ordinary English frame (the registration's shorthand for this was "all content words"), so corpus exposure is ≈ 0 by construction and verified by the same infini-gram probe protocol as the original control — **all 392 probe windows return exactly zero in both training corpora** (the registered gate only required ≤ 2; two of the 18 items yield 10 windows instead of 11 because their gold continuations are shorter). Unlike L0N, this verification does not lean on index coverage: invented lexemes cannot appear in the unindexed OLMo Stage-2 data either. Predictability is titrated across three tiers (strong / medium / weak, 6 items each): each paragraph establishes a mechanical pattern whose continuation completes it. Same five models, same measurement pipeline, run 2026-08-21 (results in `results/l0p/`, analysis in `harness/l0p_analysis.py`).

**The registered manipulation check passed in all five models** (A3: the memorized items' gold range covers 74.3–99.0% of the L0P gold range; the registered threshold was 50% in ≥ 4 of 5). The titration did what it was designed to do: never-seen text now occupies a predictability range that overlaps the memorized range over 74–99% of its span.

**Result one — the dose–response reappears on text constructed and probe-verified as corpus-absent.** Within L0P, gold log-probability tracks final-layer entropy at rho **−0.71 to −0.77 across all five runs** — against −0.70 to −0.82 within memorized L0. The registered slope comparison (A2) finds no difference it can resolve: mean Fisher-z difference **+0.050**, shared-item permutation p = **.809** (two-sided, 20,000 iterations, preregistered seed; the per-model differences split 3 positive / 2 negative). The same contrast restricted to the matched gold region — exploratory, not registered — moves to **+0.28**, memorized steeper, positive in all five runs (+0.06 / +0.41 / +0.47 / +0.36 / +0.10). The two versions disagree in the direction range restriction predicts: L0P's gold range is the wider of the two in four of five models (width ratio 0.74–1.01), the unrestricted registered comparison therefore inflates L0P's |rho|, and matching the range removes that inflation. Neither number is clean — the registered one is confounded by unequal range, the range-matched one is unregistered and lower-n — and the verdict follows the registered rule. What survives either reading: whatever produces the v0.6 dose–response does not need the text to have been in the training corpus.

**Result two — the registered test finds no memorization-specific sharpening at matched predictability; all five point estimates sit on the opposite side.** The registered primary analysis (A1: rank ANCOVA of entropy on gold plus a condition term, restricted to the gold-overlap region, within model; per-model samples n = 29–35, smallest condition cell 12) puts the condition coefficient **positive in all five runs** (+0.026 to +0.169; combined studentized statistic +1.31): at the same gold level, memorized text reads as very slightly *less* sharp than never-seen predictable text, not sharper. The combined permutation p is **.099** (two-sided) — not significant under the registered rule, and inside the fragile zone (p between .02 and .10) we pre-declared for calibration reasons. The narrative frozen before unblinding says the residual calibration error is enough to move a p in this interval either way, so we report the rule's output without claiming the reading is stable. The calibration itself ships with the repository (`harness/l0p_calibration_*.py`, stored output `results/l0p/calibration_report.txt`): the studentized statistic's false-positive rate measured .047/.053/.053 at a nominal .05 across the three standard synthetic nulls, and .085 in an adversarial null that gives the four Pythia runs shared item effects. A pre-committed exploratory leave-one-out pass (no verdict role) over the items that define each model's overlap boundary keeps p within [.083, .112]. The only nominally significant single run is Pythia-1.4B — and it is positive (+0.169, marginal p = .028).

**The registered verdict is: none of the three preregistered decision rules fired.** R-memo (a memorization-specific component survives) fails on both of its clauses: the combined p, and the sign — positive in all five runs where R-memo required negative. R-generic (the effect reads as generic predictability) fails because the preregistration required "no consistent sign in 4+ of 5 runs" — and the five signs are consistent, in the direction the memorization account does not predict. R-failed does not apply (A3 passed). Under its own frozen rules, this document therefore draws **no registered conclusion in either direction**: we do not claim the effect is memorization-specific, and we do not claim it is generic predictability. The headline is that the verdict did not arrive — the design's discriminating power, under the rules it registered, was not sufficient on this data.

**A preregistration design error, published as such.** Adversarial review of the analysis code — completed before anyone saw a result — had already shown by simulation that the R-generic exit was substantially harder to reach than the registration assumed: the four Pythia runs share one item set, their condition coefficients move together, and "no consistent sign in 4+ of 5" is therefore disfavored whatever the truth — in the simulated world where R-generic is true, this design still reports "none" more often than it reports R-generic. The registration guarded against a consistent *negative* sign (the memorization direction) and did not anticipate that a consistent *positive* sign would block the generic verdict just as effectively. The direction of this simulation result is robust; its magnitudes came from an uncalibrated generator and are not quoted. We did not amend the registration: by the time the simulation existed the runs had already executed (unread), and an amendment issued between data and unblinding is indistinguishable, to a reader, from one fitted to the data — the same reason this paper refuses to claim what it had or had not looked at. The registration also carried no prospective power or minimum-detectable-effect analysis; the firing-rate simulation was run after registration, not before. The next registration will carry both, and will define its exits over independent evidence units.

**What the two exploratory signals say together (no registered standing).** They point opposite ways for the memorization account: the A1 level effect reads against it (memorized very slightly *less* sharp, positive in all five runs), while the range-matched slope contrast reads toward it (memorized coupling steeper, positive in all five runs). Neither is registered; a design with enough discriminating power would not leave them pointing apart. Analysis-implementation decisions made before unblinding — the studentized permutation statistic after synthetic-null calibration showed the raw-coefficient version anticonservative, an exclusion rule for models with too little overlap support, and the frozen wording of every verdict branch — are logged in `docs/PROCESS_LOG.md`; the numbers above are recomputable by the reconciliation gate (`harness/reconcile.py`, `l0p.*` statistics, permutation values under an input-hashed cache), and the calibration figures regenerate from the shipped calibration scripts.

**What this does to the surviving claim.** "Memorization dose tracks output sharpness" stands as stated — and must now be read together with the fact that a tracking of similar strength, one the registered comparison cannot tell apart from it, appears on text constructed and probe-verified as corpus-absent, and that the registered test found no memorization-specific component at matched predictability at this n. The identification gap v0.6 disclosed has now been put to a registered test; the test returned no verdict, so the gap is narrowed in evidence and unresolved by the registered rules. The causal phase (activation patching), already committed to in v0.5, is unchanged and remains the arbiter.

**How to read the rest of this document.** Everything below this section is v0.6 as published, unrewritten. Its own top section (the v0.6 self-test) reversed the original depth headline; this section addresses the claim that survived that reversal. Where the v0.6 text says the memorization–sharpness link "is what the paper is left claiming," the paragraph above is the current status of that claim. (The section immediately below also opens with "read this first"; it is the v0.6 record, preserved unrewritten, and its instruction refers to the v0.6 document.)
...
- **External anchor.** Corpus duplication counts from infini-gram *[Correction 2026-08-21:
  operationally a median probe-window frequency, not a passage copy count — see
  `CORRECTION_20260821_ANCHOR.md`]* are a quantity that exists
...
Supports: a continuously graded internal correlate of parametric memory strength, one that tracks how far down the stack the layer-wise prediction runs before it converges. It is established in OLMo-2-1B, and consistent in sign across the four-model Pythia scale series, which shares one item set. Absent as a resolvable gradient both on matched novel text and on atomic facts. Externally anchored to corpus duplication counts in all five runs *[Correction 2026-08-21: read as median probe-window frequency — `CORRECTION_20260821_ANCHOR.md`]*.
